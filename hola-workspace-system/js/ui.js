...
export function renderAdminSubscriptions(_subscriptions, _plans) {
    ...
    activeList.innerHTML = active.length === 0 ? '<tr><td colspan="7" class="text-center p-4 text-gray-400 text-xs">لا توجد اشتراكات نشطة</td></tr>' :
        active.map(s => `<tr class="hover:bg-gray-50 border-b">
            ...
            <td class="p-3 text-xs text-gray-400">${s.endDate ? new Date(s.endDate).toLocaleDateString('ar-EG') : '---'}</td>
            <td class="p-3"><button onclick="window.revokeSubscription('${s.id}')" class="text-red-500 hover:text-red-700 text-xs font-bold bg-red-50 px-2 py-1 rounded hover:bg-red-100 transition"><i class="fa-solid fa-ban ml-1"></i>إلغاء</button></td>
        </tr>`).join('');
}
