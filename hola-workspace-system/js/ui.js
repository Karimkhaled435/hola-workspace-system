...
//   #4  — iframes get title via sanitizeEmbedForA11y()
//   #8  — "strong" semantic role handled in HTML
//   #31 — nav landmark corrected in index.html
//   #37 — decorative icons already have aria-hidden in FA
//   #38 — dynamic event images now have alt text
//   #43 — scroll-margin-top added to main in index.html
//   #50 — tablist role added in index.html
//   #52 — aria-selected toggled dynamically in switchClientTab/switchAdminTab
// =====================================================
...
// ─── A11y Helper: Sanitize embed code to ensure iframes have title ──────────
// FIX #4: Any dynamically injected <iframe> must have a title attribute for screen readers.
export function sanitizeEmbedForA11y(embedCode, defaultTitle = 'محتوى مضمن') {
    if (!embedCode) return embedCode;
    // Add title only to iframes that are missing one
    return embedCode.replace(
        /<iframe(?![^>]*\btitle=)([^>]*>)/gi,
        `<iframe title="${defaultTitle}"$1`
    );
}
window.sanitizeEmbedForA11y = sanitizeEmbedForA11y;
...
// FIX #52: aria-selected must be toggled on tab switches so screen readers
// announce which tab is active. Both client and admin tabs patched below.
...
export function updateCapacityUI(_sessions, sysSettings) {
    ...
    // FIX #8: use <strong> for status text that conveys emphasis/importance
    ...
    if(elStatus) elStatus.innerHTML=`<strong class="${color}">${text}</strong>`;
...
export function renderShiftManagers(sysSettings) {
    ...
    if(list) list.innerHTML=(sysSettings.shiftManagers||["مدير النظام"]).map(m=>`<span class="bg-gray-100 px-2 py-1 rounded text-sm border flex items-center gap-2">${m} <button onclick="window.removeShiftManager('${m}')" class="text-red-500 hover:text-red-700" aria-label="حذف ${m}"><i class="fa-solid fa-xmark" aria-hidden="true"></i></button></span>`).join('');
}
...
export function renderClientLoyalty(myProfile, _profiles, _discounts, sysSettings) {
    ...
    let html='';
    for(let i=0;i<req;i++){
        if(i<stamps.length) html+=`<div class="flex flex-col items-center"><div class="w-12 h-12 rounded-full bg-hola-purple text-white flex items-center justify-center text-xl shadow-lg border-2 border-hola-orange transform hover:scale-110 transition" aria-label="ختم مكتسب"><i class="fa-solid fa-stamp" aria-hidden="true"></i></div><span class="text-[10px] mt-2 font-bold text-gray-700">${new Date(stamps[i]).toLocaleDateString('ar-EG',{month:'short',day:'numeric'})}</span></div>`;
        else if(i===req-1) html+=`<div class="flex flex-col items-center"><div class="w-12 h-12 rounded-full border-2 border-dashed border-hola-orange text-hola-orange flex items-center justify-center text-xl bg-orange-50 animate-pulse" aria-label="مكافأة مجانية"><i class="fa-solid fa-gift" aria-hidden="true"></i></div><span class="text-[10px] mt-2 text-hola-orange font-black tracking-widest">مجاني!</span></div>`;
        else html+=`<div class="flex flex-col items-center"><div class="w-12 h-12 rounded-full border-2 border-dashed border-gray-300 text-gray-300 flex items-center justify-center text-xl bg-gray-50" aria-label="ختم فارغ"><i class="fa-solid fa-stamp" aria-hidden="true"></i></div><span class="text-[10px] mt-2 text-gray-400">---</span></div>`;
    }
    ...
    if(dList){
        if(myDisc.length===0) dList.innerHTML='<p class="col-span-full text-center text-sm text-gray-400 py-4">لا توجد أكواد.</p>';
            ...
            const sStr=d.isUsed?'<span class="text-red-500 font-bold text-xs"><i class="fa-solid fa-ban ml-1" aria-hidden="true"></i>مُستخدم</span>':'<span class="text-green-500 font-bold text-xs"><i class="fa-solid fa-circle-check ml-1" aria-hidden="true"></i>متاح</span>';
            const btn=d.isUsed?'':`<button onclick="window.copyToClipboard('${d.code}')" class="bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded text-xs font-bold transition" aria-label="نسخ الكود ${d.code}"><i class="fa-regular fa-copy ml-1" aria-hidden="true"></i>نسخ</button>`;
...
export function renderClientNotifications(myProfile, _notifications) {
    ...
    list.innerHTML=notifs.map(n=>{
        ...
        let extraHtml='';
        // FIX #38: images in notifications need descriptive alt text
        if(n.imgUrl) extraHtml+=`<img src="${n.imgUrl}" alt="صورة الإشعار" class="w-full max-h-48 object-cover rounded-lg mb-2 mt-2 border cursor-pointer" onclick="window.openNotifFullImg('${n.imgUrl}')" title="اضغط للعرض الكامل">`;
        // FIX #4: sanitize any embed code to ensure iframes have title
        if(n.embedCode) extraHtml+=`<div class="w-full overflow-hidden rounded-lg mt-2 border">${sanitizeEmbedForA11y(n.embedCode, 'محتوى إشعار مضمن')}</div>`;
        if(n.linkUrl){
            const platformMap = {
            ...
            extraHtml+=`<a href="${n.linkUrl}" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 ${color} text-white text-xs font-bold px-3 py-2 rounded-lg mt-2 hover:opacity-90 transition shadow-sm"><i class="${icon}" aria-hidden="true"></i> ${label}</a>`;
        }
        return `<div ${act} class="bg-gray-50 border border-gray-100 p-4 rounded-xl flex gap-4 items-start shadow-sm ${n.discountCode?'cursor-pointer hover:bg-gray-100 transition':''}"><div class="mt-1"><i class="fa-solid ${i} text-lg" aria-hidden="true"></i></div><div class="flex-grow"><p class="text-sm font-bold text-gray-800 mb-1 leading-relaxed">${msgHtml}</p>${extraHtml}<p class="text-[10px] text-gray-400 font-bold mt-1">${d}</p></div></div>`;
    }).join('');
}
...
export function showClientNotification(msg, type, docId, imgUrl, linkUrl, db, appId) {
    ...
    // FIX #38: notification images need alt text
    if(imgEl&&imgUrl){imgEl.src=imgUrl;imgEl.alt="صورة الإشعار";imgEl.classList.remove('hidden');}else if(imgEl) imgEl.classList.add('hidden');
...
export function renderClientChatMessages(myProfile, _chats) {
    ...
    list.innerHTML=myMsgs.map(m=>{
        ...
        return `<div class="max-w-[80%] px-4 py-2 rounded-2xl shadow-sm font-bold text-sm ${style}" role="article">${m.text}<span class="block text-[8px] opacity-60 mt-1">${new Date(m.timestamp).toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit'})}</span></div>`;
    }).join('');
...
export function renderAdminChatUsersList(_chats, _profiles, currentChatPhone) {
    ...
    list.innerHTML=usersWithChats.map(phone=>{
        ...
        const unreadBadge = unreadCount > 0 && currentChatPhone !== phone
            ? `<span class="bg-red-500 text-white text-[9px] px-1.5 py-0.5 rounded-full font-black ml-1" aria-label="${unreadCount} رسائل غير مقروءة">${unreadCount}</span>`
            : '';
        const timeStr = lastMsg ? new Date(lastMsg.timestamp).toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit'}) : '';
        return `<div onclick="window.openAdminChat('${phone}')" class="p-3 border-2 rounded-xl cursor-pointer transition ${isActive}" role="button" tabindex="0" onkeydown="if(event.key==='Enter'||event.key===' ')window.openAdminChat('${phone}')">
    ...
    if(list){
        ...
        if(active.length===0) list.innerHTML='<tr><td colspan="4" class="text-center p-4 text-gray-500">لا يوجد نشاط حالي</td></tr>';
            ...
            const statusBadge = isRemote
                ...
                : '<span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs font-bold"><span class="inline-block w-2 h-2 bg-green-500 rounded-full mr-1" aria-hidden="true"></span>داخل المكان</span>';
...
export function renderAdminPreBookings(_prebookings) {
    ...
    if(archBookings.length===0){archList.innerHTML='<tr><td colspan="4" class="text-center p-4 text-gray-500 text-xs">لا يوجد أرشيف</td></tr>';}
        ...
        const tDate=b.expectedTime==='سيتم التنسيق'?b.expectedTime:new Date(b.expectedTime).toLocaleString('ar-EG',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
        return `<tr class="hover:bg-gray-50"><td class="p-3 font-bold">${b.name}</td><td class="p-3 font-mono text-xs">${b.phone}</td><td class="p-3 text-xs" dir="ltr">${tDate}</td><td class="p-3 text-green-600 text-xs font-bold"><i class="fa-solid fa-check-double" aria-hidden="true"></i> تم</td></tr>`;
    }).join('');
}
...
export function renderAdminUsers(_profiles) {
    ...
    list.innerHTML=sortedUsers.map(u=>`<tr class="hover:bg-gray-50 border-b"><td class="p-3 font-bold text-hola-purple">${u.name}</td><td class="p-3 font-mono text-xs">${u.phone}</td><td class="p-3 font-black text-hola-orange"><i class="fa-solid fa-stamp text-[10px] mr-1" aria-hidden="true"></i> ${u.stamps?.length||0}</td><td class="p-3 font-bold text-green-600">${u.walletBalance||0} ج</td><td class="p-3"><div class="flex gap-1 justify-end"><button onclick="window.openUserManage('${u.phone}')" class="bg-gray-800 text-white text-[10px] px-2 py-1.5 rounded hover:bg-black transition" aria-label="إجراء على ${u.name}"><i class="fa-solid fa-bolt" aria-hidden="true"></i> إجراء</button><button onclick="window.openUserDetails('${u.phone}')" class="bg-hola-purple text-white text-[10px] px-2 py-1.5 rounded hover:bg-hola-dark transition" aria-label="تفاصيل ${u.name}"><i class="fa-solid fa-eye" aria-hidden="true"></i> تفاصيل</button></div></td></tr>`).join('');
}
...
export function renderAdminBanned(_bannedPhones) {
    ...
    if(banned.length===0){list.innerHTML='<tr><td colspan="4" class="text-center p-4 text-gray-500 text-xs">لا يوجد أرقام محظورة حالياً</td></tr>';}
        ...
        return `<tr class="hover:bg-red-50 transition"><td class="p-3 font-bold font-mono text-red-600" dir="ltr">${b.phone}</td><td class="p-3 text-xs" dir="ltr">${tDate}</td><td class="p-3 text-xs text-gray-500">${b.reason}</td><td class="p-3"><button onclick="window.unbanPhone('${b.phone}')" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-[10px] font-bold shadow" aria-label="فك حظر ${b.phone}"><i class="fa-solid fa-unlock" aria-hidden="true"></i> فك الحظر</button></td></tr>`;
    }).join('');
}
...
export function renderPublicEvents(sysSettings, myProfile, activeSessionId) {
    ...
    for (let slot = 1; slot <= 3; slot++) {
        ...
        if (sysSettings[key('evActive')] && sysSettings[key('evTitle')]) {
            ...
            // FIX #4: sanitize embed code so iframes get title attributes
            const embedSanitized = sanitizeEmbedForA11y(sysSettings[key('evEmbed')], `محتوى فعالية: ${sysSettings[key('evTitle')]}`);
            html += `
                <div class="glass-card p-4 border border-purple-200 shadow-md relative overflow-hidden cursor-pointer hover:shadow-lg transition" onclick="window.openEventDetails(${slot})">
                    <div class="absolute top-0 right-0 bg-hola-purple text-white text-[10px] px-3 py-1 rounded-bl-lg font-bold"><i class="fa-solid fa-masks-theater mr-1" aria-hidden="true"></i> فعالية</div>
                    <div class="flex items-center gap-3 mt-2">
                        ${embedSanitized
                            ? `<div class="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 border border-purple-100">${embedSanitized}</div>`
                            : sysSettings[key('evImg')]
                                ...
                                : `<div class="w-14 h-14 bg-purple-100 rounded-lg flex items-center justify-center text-2xl flex-shrink-0" aria-hidden="true"><i class="fa-solid fa-calendar-star text-hola-purple"></i></div>`}
                        <div class="flex-1">
                            <h4 class="font-black text-hola-purple text-sm">${sysSettings[key('evTitle')]}</h4>
                            <p class="text-xs text-gray-500 font-bold mt-0.5"><i class="fa-regular fa-clock ml-1" aria-hidden="true"></i>${sysSettings[key('evTime')] || 'اضغط لمعرفة التفاصيل'}</p>
                        </div>
                        <div class="flex flex-col gap-1">
                            ...
                            <button onclick="event.stopPropagation();window.shareClientEvent(${slot})" class="bg-blue-50 text-blue-600 text-[10px] px-3 py-1.5 rounded-lg font-bold hover:bg-blue-100 transition border border-blue-200 whitespace-nowrap"><i class="fa-solid fa-share-nodes ml-1" aria-hidden="true"></i> مشاركة</button>
                        </div>
                    </div>
                </div>`;
        }
    ...
    if (clientBanner && firstActiveEvent && myProfile && activeSessionId) {
        ...
        // FIX #38: set meaningful alt text on event banner image
        const bannerImg = document.getElementById('eventBannerImg');
        if (bannerImg) {
            bannerImg.src = firstActiveEvent.img || '';
            bannerImg.alt = `صورة فعالية: ${firstActiveEvent.title}`;
        }
        ...
        ...pending.map(s => `
            <div class="bg-yellow-50 border-2 border-yellow-200 p-4 rounded-2xl">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-yellow-100 text-yellow-600 rounded-full flex items-center justify-center"><i class="fa-solid fa-clock" aria-hidden="true"></i></div>
    ...
    pendingList.innerHTML = pending.length === 0 ? '<tr><td colspan="5" class="text-center p-4 text-gray-400 text-xs">لا توجد طلبات معلقة</td></tr>' :
        pending.map(s => `<tr class="hover:bg-gray-50 border-b">
            ...
            <td class="p-3"><div class="flex gap-1 flex-wrap">
                <button onclick="window.approveSubscription('${s.id}')" class="bg-green-500 text-white text-[10px] px-2 py-1.5 rounded font-bold hover:bg-green-600 transition shadow-sm" aria-label="قبول اشتراك ${s.name}"><i class="fa-solid fa-check ml-1" aria-hidden="true"></i>قبول وتفعيل</button>
                <button onclick="window.rejectSubscription('${s.id}')" class="bg-red-500 text-white text-[10px] px-2 py-1.5 rounded font-bold hover:bg-red-600 transition" aria-label="رفض اشتراك ${s.name}"><i class="fa-solid fa-ban ml-1" aria-hidden="true"></i>رفض</button>
            </div></td>
        </tr>`).join('');
        ...
        active.map(s => `<tr class="hover:bg-gray-50 border-b">
            ...
            <td class="p-3">
                <div class="flex items-center gap-1">
                    <label class="sr-only" for="days-${s.id}">الأيام المتبقية لـ ${s.name}</label>
                    <input id="days-${s.id}" type="number" value="${s.daysLeft || 0}" min="0" max="365"
            ...
            <td class="p-3"><div class="flex gap-1 flex-wrap">
                <button onclick="window.showSubCard('${s.id}')" class="bg-purple-100 text-hola-purple text-[10px] px-2 py-1 rounded font-bold hover:bg-purple-200 transition" aria-label="عرض بطاقة ${s.name}"><i class="fa-solid fa-id-card ml-1" aria-hidden="true"></i>بطاقة</button>
                <button onclick="window.pauseSubscriptionNow('${s.id}')" class="bg-yellow-100 text-yellow-700 text-[10px] px-2 py-1 rounded font-bold hover:bg-yellow-200 transition" aria-label="إيقاف اشتراك ${s.name}"><i class="fa-solid fa-pause ml-1" aria-hidden="true"></i>إيقاف</button>
                <button onclick="window.schedulePauseSubscription('${s.id}')" class="bg-orange-100 text-orange-700 text-[10px] px-2 py-1 rounded font-bold hover:bg-orange-200 transition" aria-label="جدولة إيقاف ${s.name}"><i class="fa-solid fa-calendar-xmark ml-1" aria-hidden="true"></i>جدولة</button>
                <button onclick="window.revokeSubscription('${s.id}')" class="text-red-500 hover:text-red-700 text-[10px] font-bold bg-red-50 px-2 py-1 rounded hover:bg-red-100 transition" aria-label="إلغاء اشتراك ${s.name}"><i class="fa-solid fa-ban ml-1" aria-hidden="true"></i>إلغاء</button>
            </div></td>
        </tr>`).join('');
}
        ...
        plans.map(p => `
            <div class="border-2 ${p.active !== false ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50 opacity-60'} rounded-2xl p-4 relative hover:shadow-md transition">
                <div class="absolute top-2 left-2 flex gap-1">
                    <button onclick="window.togglePlanActive('${p.id}', ${p.active !== false})" class="text-[10px] px-2 py-1 rounded font-bold ${p.active !== false ? 'bg-green-100 text-green-700 hover:bg-red-100 hover:text-red-700' : 'bg-gray-200 text-gray-600 hover:bg-green-100 hover:text-green-700'} transition" aria-label="${p.active !== false ? 'إيقاف' : 'تفعيل'} باقة ${p.name}" aria-pressed="${p.active !== false}">
                        ${p.active !== false ? '<i class="fa-solid fa-toggle-on" aria-hidden="true"></i> نشط' : '<i class="fa-solid fa-toggle-off" aria-hidden="true"></i> متوقف'}
                    </button>
                    <button onclick="window.deletePlan('${p.id}')" class="text-red-400 hover:text-red-600 bg-red-50 w-6 h-6 rounded flex items-center justify-center" aria-label="حذف باقة ${p.name}"><i class="fa-solid fa-trash text-xs" aria-hidden="true"></i></button>
                </div>
