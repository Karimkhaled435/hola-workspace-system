...
export function renderClientLoyalty(myProfile, _profiles, _discounts, sysSettings) {
    ...
    if(dList){
        if(myDisc.length===0) dList.innerHTML='<p class="col-span-full text-center text-sm text-gray-400 py-4">لا توجد أكواد.</p>';
            ...
            const sStr=d.isUsed?'<span class="text-red-500 font-bold text-xs"><i aria-hidden="true" class="fa-solid fa-ban ml-1"></i>مُستخدم</span>':'<span class="text-green-500 font-bold text-xs"><i aria-hidden="true" class="fa-solid fa-circle-check ml-1"></i>متاح</span>';
            const btn=d.isUsed?'':`<button onclick="window.copyToClipboard('${d.code}')" class="bg-gray-100 hover:bg-gray-200 px-3 py-1 rounded text-xs font-bold transition"><i aria-hidden="true" class="fa-regular fa-copy ml-1"></i>نسخ</button>`;
...
export function renderClientNotifications(myProfile, _notifications) {
    ...
    list.innerHTML=notifs.map(n=>{
        ...
        let extraHtml='';
        if(n.imgUrl) extraHtml+=`<img src="${n.imgUrl}" alt="صورة مرفقة مع الإشعار" class="w-full max-h-48 object-cover rounded-lg mb-2 mt-2 border cursor-pointer" onclick="window.openNotifFullImg('${n.imgUrl}')" title="اضغط للعرض الكامل">`;
        if(n.embedCode) {
            let safeEmbed = n.embedCode;
            if(!safeEmbed.includes('title=')) safeEmbed = safeEmbed.replace('<iframe', '<iframe title="محتوى مضمن"');
            extraHtml+=`<div class="w-full overflow-hidden rounded-lg mt-2 border">${safeEmbed}</div>`;
        }
        if(n.linkUrl){
            ...
            extraHtml+=`<a href="${n.linkUrl}" target="_blank" class="inline-flex items-center gap-2 ${color} text-white text-xs font-bold px-3 py-2 rounded-lg mt-2 hover:opacity-90 transition shadow-sm"><i aria-hidden="true" class="${icon}"></i> ${label}</a>`;
        }
        return `<div ${act} class="bg-gray-50 border border-gray-100 p-4 rounded-xl flex gap-4 items-start shadow-sm ${n.discountCode?'cursor-pointer hover:bg-gray-100 transition':''}"><div class="mt-1"><i aria-hidden="true" class="fa-solid ${i} text-lg"></i></div><div class="flex-grow"><p class="text-sm font-bold text-gray-800 mb-1 leading-relaxed">${msgHtml}</p>${extraHtml}<p class="text-[10px] text-gray-400 font-bold mt-1">${d}</p></div></div>`;
    }).join('');
}
...
export function renderAdminPreBookings(_prebookings) {
    ...
    if(archBookings.length===0){archList.innerHTML='<tr><td colspan="4" class="text-center p-4 text-gray-500 text-xs">لا يوجد أرشيف</td></tr>';}
        ...
        const tDate=b.expectedTime==='سيتم التنسيق'?b.expectedTime:new Date(b.expectedTime).toLocaleString('ar-EG',{month:'short',day:'numeric',hour:'2-digit',minute:'2-digit'});
        return `<tr class="hover:bg-gray-50"><td class="p-3 font-bold">${b.name}</td><td class="p-3 font-mono text-xs">${b.phone}</td><td class="p-3 text-xs" dir="ltr">${tDate}</td><td class="p-3 text-green-600 text-xs font-bold"><i aria-hidden="true" class="fa-solid fa-check-double"></i> تم</td></tr>`;
    }).join('');
}
...
export function renderAdminMenu(_menu) {
    ...
    list.innerHTML=Object.values(_menu).map(item=>`<div class="border border-gray-200 rounded-xl p-4 flex flex-col items-center justify-center text-center relative hover:shadow-md transition"><button aria-label="حذف العنصر" onclick="window.deleteMenuItem('${item.id}')" class="absolute top-2 right-2 text-red-400 hover:text-red-600"><i aria-hidden="true" class="fa-solid fa-trash"></i></button><i aria-hidden="true" class="fa-solid ${item.icon||'fa-star'} text-2xl text-hola-purple mb-2"></i><p class="font-bold text-sm mb-1">${item.name}</p><p class="text-hola-orange font-black text-sm">${item.price} ج</p><span class="text-[10px] bg-gray-100 px-2 py-0.5 rounded mt-2">${item.type==='drink'?'مشروب':'واي فاي'}</span></div>`).join('');
}
...
export function renderAdminUsers(_profiles) {
    ...
    list.innerHTML=sortedUsers.map(u=>`<tr class="hover:bg-gray-50 border-b"><td class="p-3 font-bold text-hola-purple">${u.name}</td><td class="p-3 font-mono text-xs">${u.phone}</td><td class="p-3 font-black text-hola-orange"><i aria-hidden="true" class="fa-solid fa-stamp text-[10px] mr-1"></i> ${u.stamps?.length||0}</td><td class="p-3 font-bold text-green-600">${u.walletBalance||0} ج</td><td class="p-3"><div class="flex gap-1 justify-end"><button aria-label="إدارة المستخدم" onclick="window.openUserManage('${u.phone}')" class="bg-gray-800 text-white text-[10px] px-2 py-1.5 rounded hover:bg-black transition"><i aria-hidden="true" class="fa-solid fa-bolt"></i> إجراء</button><button aria-label="تفاصيل المستخدم" onclick="window.openUserDetails('${u.phone}')" class="bg-hola-purple text-white text-[10px] px-2 py-1.5 rounded hover:bg-hola-dark transition"><i aria-hidden="true" class="fa-solid fa-eye"></i> تفاصيل</button></div></td></tr>`).join('');
}
...
export function renderPublicEvents(sysSettings, myProfile, activeSessionId) {
    ...
    for (let slot = 1; slot <= 3; slot++) {
        ...
        if (sysSettings[key('evActive')] && sysSettings[key('evTitle')]) {
            if (!firstActiveEvent) firstActiveEvent = { slot, title: sysSettings
