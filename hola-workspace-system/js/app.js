...
export async function submitPreBooking() {
    ...
    try {
        ...
        document.getElementById('preBookingForm').innerHTML = `<div class="text-center py-6"><div class="w-20 h-20 bg-green-100 text-green-600 rounded-full flex items-center justify-center text-4xl mx-auto mb-4"><i class="fa-solid fa-check-double" aria-hidden="true"></i></div><h3 class="text-xl font-black text-hola-purple mb-2">تم استلام حجزك بنجاح!</h3><p class="text-gray-600 font-bold leading-relaxed">سوف نقوم بالتواصل معك خلال ساعه من الان.</p><button data-action="switchView" data-view="public" onclick="location.reload()" class="mt-6 text-hola-orange font-bold text-sm hover:underline">العودة للرئيسية</button></div>`;
...
export async function handleLogin() {
    ...
    const isNewUserUI = nameField && !nameField.classList.contains('hidden');
    ...
    if (recaptchaResponse.length === 0) {
        window._loginAttempts[p] = (window._loginAttempts[p] || 0) + 1;
        if (window._loginAttempts[p] >= 3) {
            ...
            showMsg("برجاء تأكيد أنك لست روبوت (reCAPTCHA)", "error");
        }
        return;
    }
