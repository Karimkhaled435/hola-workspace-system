...
export function renderAdminChatUsersList(_chats, _profiles, currentChatPhone) {
    ...
    list.innerHTML=usersWithChats.map(phone=>{
        ...
        return `<div onclick="window.openAdminChat('${phone}')" class="p-3 border-2 rounded-xl cursor-pointer transition ${isActive}"><h4 class="text-sm font-black text-hola-orange">${name}</h4><p class="text-xs truncate ${bold}">${lastMsg.text}</p></div>`;
    }).join('');
...
export function renderAdminSessions(_sessions, _profiles) {
    ...
    if(list){
        ...
        else list.innerHTML=active.map(s=>`<tr class="hover:bg-gray-50 border-b transition cursor-pointer" onclick="window.openAdminLiveSession('${s.id}')"><td class="p-3 font-bold text-hola-purple">${s.name||s.phone.substring(0,6)}</td><td class="p-3 text-gray-500 text-xs" dir="ltr">${new Date(s.startTime).toLocaleTimeString('ar-EG',{hour:'2-digit',minute:'2-digit'})}</td><td class="p-3 text-hola-orange font-mono font-bold admin-timer" data-start="${s.startTime}">00:00</td><td class="p-3"><span class="bg-green-100 text-green-700 px-2 py-1 rounded text-xs">نشط</span></td></tr>`).join('');
    }
            ...
            html += `
                <div class="glass-card p-4 border border-purple-200 shadow-md relative overflow-hidden cursor-pointer hover:shadow-lg transition" onclick="window.openEventDetails(${slot})">
                    ...
                    <div class="flex items-center gap-3 mt-2">
                        ${sysSettings[key('evImg')] ? `<img src="${sysSettings[key('evImg')]}" class="w-14 h-14 rounded-lg object-cover border border-purple-100 flex-shrink-0">` : `<div class="w-14 h-14 bg-purple-100 rounded-lg flex items-center justify-center text-2xl flex-shrink-0"><i class="fa-solid fa-calendar-star text-hola-purple"></i></div>`}
                        <div class="flex-1">
...
export function renderClientSubscriptions(myProfile, _subscriptions) {
    ...
    if (mySubs.length === 0) { list.innerHTML = '<p class="text-center text-gray-400 text-sm py-4">لا توجد اشتراكات</p>'; return; }
    ...
    pendingList.innerHTML = pending.length === 0 ? '<tr><td colspan="5" class="text-center p-4 text-gray-400 text-xs">لا توجد طلبات معلقة</td></tr>' :
        pending.map(s => `<tr class="hover:bg-gray-50 border-b">
            ...
            <td class="p-3"><div class="flex gap-1">
                <button onclick="window.approveSubscription('${s.id}')" class="bg-green-500 text-white text-[10px] px-2 py-1.5 rounded font-bold hover:bg-green-600"><i class="fa-solid fa-check ml-1"></i>قبول وتفعيل</button>
                <button onclick="window.revokeSubscription('${s.id}')" class="bg-red-500 text-white text-[10px] px-2 py-1.5 rounded font-bold hover:bg-red-600"><i class="fa-solid fa-ban"></i></button>
            </div></td>
        </tr>`).join('');
    activeList.innerHTML = active.length === 0 ? '<tr><td colspan="6" class="text-center p-4 text-gray-400 text-xs">لا توجد اشتراكات نشطة</td></tr>' :
        active.map(s => `<tr class="hover:bg-gray-50 border-b">
            <td class="p-3 font-bold text-hola-purple">${s.name}</td>
            <td class="p-3 font-mono text-xs">${s.phone}</td>
            <td class="p-3 text-xs font-bold text-hola-orange">${s.planName}</td>
            <td class="p-3 font-mono text-xs font-bold text-hola-purple">${s.code || '---'}</td>
            <td class="p-3"><span class="font-black text-lg text-hola-orange">${s.daysLeft || 0}</span><span class="text-xs text-gray-400 ml-1">يوم</span></td>
            <td class="p-3"><button onclick="window.revokeSubscription('${s.id}')" class="text-red-500 hover:text-red-700 text-xs font-bold"><i class="fa-solid fa-ban"></i> إلغاء</button></td>
        </tr>`).join('');
}
